...
#FIXME
    ...
    def curses_print(self, *args, **kwargs):
        """Redirect print(...) to the curses status window."""
        # original print goes to terminal
        self._orig_print(*args, **kwargs)
        msg = ' '.join(str(a) for a in args)
        # add line to status, scroll if needed
    ...
    def destroy_node(self):
        # restore terminal so we don’t break your shell
        termios.tcsetattr(sys.stdin, termios.TCSADRAIN, self._orig_tty)
        # restore curses and original print
...
#FIXME
            ...
            t_tr  = self.get_current_trajectory_time()        # now = ros.now - self.t_tr0


            if t_tr <= (self.Tpi_spin[-1] + self.t_lg):
                ...
                t_ref = min(t_tr, self.Tpi_spin[-1])
