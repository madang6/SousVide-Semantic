...
#
        ...
        self.offboard_allow_edit_pub = self.create_publisher(
            Bool, drone_prefix + '/offboard_helper/allow_edit', qos_drone
        )
...
#async vision loop
        ...
        self.latest_decision_depth_mask = None
...
#
    ...
    def offboard_query_cb(self, msg: String):
        # Update the current prompt to what offboard used
        self.prompt = msg.data
        print(f"[offboard] Updated prompt from helper: '{self.prompt}'")

    def _publish_handshake(self, value: bool):
        try:
            self.offboard_handshake_pub.publish(Bool(data=value))
        except Exception:
            pass

    def _publish_allow_edit(self, value: bool):
        try:
            self.offboard_allow_edit_pub.publish(Bool(data=value))
        except Exception:
            pass

    def _pulse_handshake(self):
        # rising edge is what the HELPER cares about
    ...
    def async_vision_loop(self):
        ...
        while not self.vision_shutdown:
            ...
            # self.latest_decision_depth_mask = decision_data['depth_mask']
            self.latest_decision_depth_mask = decision_data['overlap']
...
#FIXME        
        if self.key_pressed == '\x1b':     # ESC
            ...
            self.t_tr0 = self.get_clock().now().nanoseconds/1e9             # Record start time
            self.early_exit   = False
            self._pulse_handshake()
            self._publish_allow_edit(True)
            zch.engage_offboard_control_mode(self.get_current_timestamp_time(),self.vehicle_command_publisher)
        ...
        if self.sm == StateMachine.INIT:
            ...
            if t_tr < (self.Tpi[-1]+self.t_lg) and not self.early_exit:
                ...
                # self.recorder.record(self.latest_decision_depth_mask,t_tr,u_act,x_ref,u_ref,x_est,x_ext,adv,t_sol)
            else:
                if self.early_exit:
                    print("\033[95m📍 HOVERING!!!, Close to query ✨\033[0m")
                    self.recorder.record(self.latest_decision_depth_mask)
        ...
        #NOTE THIS SHOULD BE REPLACED WITH A NEWER VERSION OF THE READY STATE, BUT FOR NOW IT SUFFICES
            if (self.ready_active is True and self.spin_cycle is False) and t_tr > 3.0 and self.prompt.lower() != self.hold_prompt.lower():
                ...
                zch.engage_offboard_control_mode(self.get_current_timestamp_time(),self.vehicle_command_publisher)
                self._pulse_handshake()
                self._publish_allow_edit(False)
                self.sm = StateMachine.SPIN
